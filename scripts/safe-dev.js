...
// Usage: node scripts/safe-dev.js <command> [args...]
...
const audit = spawnSync(pm, ["audit", "--audit-level=high"], { stdio: "inherit" });
if (audit.status !== 0) {
  console.error(
...
if (cmd.length === 0) {
  console.error("[safe-dev] Usage: node scripts/safe-dev.js <command> [args...]");
  process.exit(2);
}
