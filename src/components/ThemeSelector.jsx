  ...
  return (
    <div ref={menuRef}>
      <div
        className={
          isMobile
            ? 'fixed right-1 top-4 z-20'
            : 'md:block hidden fixed top-4 z-30 hide animate-slide-left'
        }
        ref={ref}
      >
        <div className="relative">
          ...
          {isOpen && (
            <div
              className={`absolute -top-5 mt-2 p-2 rounded-md ${isMobile ? '-left-36' : 'left-16'}`}
            >
              <div className="flex flex-col custom-icon">
                {availableThemes.map((t) => (
                  ...
                  >
                    <span>
                      {t === 'dark' ? (
                        <FiMoon size={20} className="custom-icon" />
                      ) : t === 'system' ? (
                        <FiSettings size={20} className="custom-icon" />
                      ) : (
                        <FiSun size={20} className="custom-icon" />
                      )}
                    </span>
